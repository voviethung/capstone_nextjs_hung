...
const Search = async (props) => {
    ...
    return (
        <div className='container'>
            ...
            <h3>Results for html ({data.length})</h3>

            {/* Toggle between Freelancers and Services */}
            {/* <div className="btn-group mb-3" role="group">
                <button
                    type="button"
                    className={`btn ${isFreelancer ? 'btn-light' : 'btn-outline-secondary'}`}
                    onClick={() => setIsFreelancer(true)}
                >
                    <i className="bi bi-person"></i> Freelancers
                </button>
                <button
                    type="button"
                    className={`btn ${!isFreelancer ? 'btn-light' : 'btn-outline-secondary'}`}
            ...
            <div className="d-flex justify-content-between align-items-center mt-3">
                ...
                <span className="text-muted">22,000+ results</span>
            ...
            <div className='row'>
                {data.map((cV) => {
                    return <div className='col-md-3 mt-2' key={cV.id}>
                        <div className='card'>
                            {/* <Image src={prod.image} alt={prod.alias} quality={100} crossOrigin={'anonymous'} width={500} height={500} style={{ width: '100%', height: 'auto' }} /> */}
                            <div className='card-body'>
                                <h3>{cV.congViec.tenCongViec}</h3>
